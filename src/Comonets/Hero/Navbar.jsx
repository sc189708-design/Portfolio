import { a, button, div, link } from "framer-motion/client";
import React, { useState } from "react";
import { color, motion, scale, time } from "framer-motion";
import { Menu, Moon, Sun, X } from "lucide-react";

export default function Nav({ darkMode, toggelDark }) {

    const [activeSection, setActiveSection] = useState('home');
    const [isManu, setIsMenuOpen] = useState(false)
    ...
    return (
        <div className=" flex justify-center w-full fixed z-50 mt-4">
            <motion.nav
                ...
                className={`flex items-center justify-center ${colors.navBg} backdrop-blur-lg
                    ...
                    <div className=" hidden lg:flex items-center space-x-6">
                        {navItems.map((items) => (
                            <a
                                key={items.name}
                                href={items.link}
                                onClick={() => handelNavClick(items.name)}
                                className="relative"
                            >
                                <motion.span
                                    className={`font-medium transition-colors duration-300 ${activeSection === items.name.toLowerCase()
                                        ? colors.textActive
